...
namespace InsurgencyWeapons.Projectiles
{
    internal abstract class WeaponBase : ModProjectile
    {
        ...
        public void DrawMuzzleFlash(Color color, float offset, float scale, Vector2 jankFix)
        {
            ...
            {
                Vector2 muzzleDrawPos = Player.RotatedRelativePoint((Player.MountedCenter + jankFix) + (Player.MountedCenter.DirectionTo(MouseAim) * offset));
                Texture2D muzzleFlash = HelperStats.MuzzleFlash;
                Rectangle rect = muzzleFlash.Frame(verticalFrames: 6, frameY: ShotDelay);
                Main.EntitySpriteDraw(muzzleFlash, muzzleDrawPos - Main.screenPosition, rect, color, Projectile.rotation + (MathHelper.PiOver2 * -Player.direction), rect.Size() / 2, scale, (SpriteEffects)(Player.direction > 0 ? 0 : 1), 0);
            }
        }
        ...
        public override void AI()
        {
            ...
            if (Player.channel || ReloadTimer > 0 || UnderAlternateFireCoolDown || Insurgency.Rifles.Contains(HeldItem.type))
            {
                ...
                Vector2 recoilVertical = Vector2.UnitY * (-ShotDelay / 6f);
